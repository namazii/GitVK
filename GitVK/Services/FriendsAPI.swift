...
class FriendsAPI {
    ...
    func fetchFriends(offset: Int = 0) async throws -> [Friend] {
        do {
            let friendsResponse = try await API.request(endpoint: FriendsEndpoint.fetchFriends(offset: offset), responseModel: FriendResponse.self)
            let friends = friendsResponse.items
            return friends
        } catch {
            throw error
        }
    ...
    func searchFriends(offset: Int = 0, search: String = "") async throws -> [Friend] {
        do {
            let friendsResponse = try await API.request(endpoint: FriendsEndpoint.searchFriends(offset: offset, search: search), responseModel: FriendResponse.self)
            let friends = friendsResponse.items
            return friends
        } catch {
            throw error
    ...
    func searchFriends(offset: Int = 0, search: String, completion: @escaping([Friend]) -> ()) {
        
        var urlComponents = URLComponents()
        urlComponents.scheme = "https"
        urlComponents.host = "api.vk.com"
        urlComponents.path = "/method/friends.search"
